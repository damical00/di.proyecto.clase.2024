...
namespace di.proyecto.clase._2024.Backend.Servicios
{
    ...
    public class UsuarioServicio : ServicioGenerico<Usuario>
    {
        ...
        public UsuarioServicio(DiinventarioexamenContext context) : base(context)
        {
            contexto = context;
        }
        /// <summary>
        ...
        public async Task<Boolean> Login(String user, String pass)
        {
            ...
            if (usuLogin != null && usuLogin.Username.Equals(user) && usuLogin.Password.Equals(pass))
            {
                correcto = true;
            }
            return correcto;
        ...
        public async Task <bool> UsuarioUnico(string usu)
        {
            bool unico = true;
            // Buscamos el usuario, si la lista no está vacía, entonces
            // ponemos la variable a false
            IEnumerable<Usuario> lista = await FindAsync(u => u.Username.Equals(usu));
            if(contexto.Set<Usuario>().Where(u => u.Username.Equals(u)).Count()>0)
            {
                unico = false;
            }
            return unico;
        ...
        public async Task<Usuario> GetUsuarioPorNombre(string nom)
        {
            ...
            Usuario usu = null;
            usuarios = await FindAsync(u => u.Username == nom);
            if (usuarios !=  null)
            {
        ...
        public async Task<IEnumerable<Usuario>> GetProfesores()
        {
            var profesores = await FindAsync(u => u.Tipo == PROFESOR);
            return (IEnumerable<Usuario>)profesores;
        }
